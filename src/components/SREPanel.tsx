...
import {
  Activity, Brain, Shield, Server, BookOpen,
  AlertTriangle, CheckCircle2, Clock, Zap, Eye, Lock,
  Terminal, MessageSquare, FileText, Layers,
  Radio, Gauge, HeartPulse, Bot, Workflow,
  ShieldCheck, ChevronRight, Timer,
  Play, Pause, CircleDot, ExternalLink, Link2,
} from "lucide-react";
...
const INTEGRATIONS: Integration[] = [
  {
    ...
    status: "available",
    description: "Auto-create and update Jira issues on DEPLOY_FAILED, ROLLBACK_TRIGGERED, and DRIFT_DETECTED events. Link incidents to sprints and epics.",
    eventTypes: ["DEPLOY_FAILED", "ROLLBACK_TRIGGERED", "DRIFT_DETECTED"],
    icon: "🔷",
    tier: "native",
  },
  {
    ...
    category: "Ticketing / ITSM",
    status: "available",
    description: "Create Linear issues for infrastructure incidents. Map severity to priority, assign to teams, and track resolution in cycles.",
    eventTypes: ["DEPLOY_FAILED", "ROLLBACK_TRIGGERED", "DRIFT_DETECTED", "COHERENCE_VIOLATION"],
    icon: "◆",
    tier: "community",
  },
  {
    ...
    status: "connected",
    description: "Real-time incident notifications, HITL approval workflows, deployment status updates, and drift alerts to designated channels.",
    eventTypes: ["DEPLOY_COMPLETE", "DEPLOY_FAILED", "PREFLIGHT_FAILED", "DRIFT_DETECTED", "ROLLBACK_TRIGGERED"],
    icon: "💬",
    tier: "native",
  },
  {
    ...
    category: "Communication",
    status: "available",
    description: "Push incident alerts and deployment summaries to Teams channels. Support for adaptive cards with action buttons for HITL approval.",
    eventTypes: ["DEPLOY_COMPLETE", "DEPLOY_FAILED", "DRIFT_DETECTED", "ROLLBACK_TRIGGERED"],
    icon: "🟣",
    tier: "community",
  },
  {
    ...
    category: "ITSM",
    status: "available",
    description: "Create and update ServiceNow change requests and incidents. Map ZTAI records to CMDB entries. Full ITIL compliance workflow.",
    eventTypes: ["DEPLOY_COMPLETE", "DEPLOY_FAILED", "ROLLBACK_TRIGGERED", "PREFLIGHT_COMPLETE"],
    icon: "🟢",
    tier: "native",
  },
];
...
function integrationStatusBadge(status: Integration["status"]) {
  switch (status) {
    case "connected": return <Badge className="bg-[hsl(var(--success))]/15 text-[hsl(var(--success))] border-[hsl(var(--success))]/30 text-[10px]">Connected</Badge>;
    case "available": return <Badge variant="outline" className="text-[10px] border-primary/30 text-primary">Available</Badge>;
    case "coming-soon": return <Badge variant="outline" className="text-[10px] text-muted-foreground">Coming Soon</Badge>;
  }
}
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="incidents" className="space-y-4">
        <TabsList className="bg-muted/50 border border-border h-auto flex-wrap">
        ...
        <TabsContent value="integrations" className="space-y-4">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {INTEGRATIONS.map((integ) => (
              <Card key={integ.id} className={`border-border transition-all hover:border-primary/30 ${integ.status === "connected" ? "ring-1 ring-[hsl(var(--success))]/20" : ""}`}>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-xs text-muted-foreground">{integ.category}</span>
                      </div>
                    </div>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {integ.eventTypes.map((evt) => (
                      <Badge key={evt} variant="outline" className="text-[9px] font-mono border-border">
                        {evt}
                      </Badge>
                    ))}
                  </div>
                  <div className="flex items-center justify-between pt-1">
                    <Badge variant="outline" className="text-[9px]">
                      {integ.tier === "native" ? "Tier 1 — Native" : "Tier 2 — Community"}
                    </Badge>
          ...
          <Card className="border-border">
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2">
                ...
                Naawi Integration Framework (NIF)
              </CardTitle>
              <CardDescription>
                PRD §5.5 — ZTAI events route to enterprise tools via webhook-based event bus with provider-specific formatters
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex flex-wrap items-center gap-2 text-xs font-medium">
                {[
                  { label: "ZTAI Event", icon: <Shield className="h-3.5 w-3.5" /> },
                  { label: "NIF Event Bus", icon: <Workflow className="h-3.5 w-3.5" /> },
                  { label: "Provider Formatter", icon: <FileText className="h-3.5 w-3.5" /> },
                  { label: "Webhook Delivery", icon: <ExternalLink className="h-3.5 w-3.5" /> },
                  { label: "Ack / Retry", icon: <CheckCircle2 className="h-3.5 w-3.5" /> },
                ].map((step, i, arr) => (
            ...
            <CardContent>
              <div className="flex flex-wrap items-center gap-2 text-xs font-medium">
                {[
                  ...
                  { label: "HITL Gate", icon: <Shield className="h-3.5 w-3.5" /> },
                  { label: "Remediate (Write)", icon: <Zap className="h-3.5 w-3.5" /> },
          ...
          <Card className="border-border border-[hsl(var(--warning))]/30">
            <CardContent className="p-4">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                ...
                trigger a Slack/Teams approval workflow before the MCP Host executes. The agent pauses, generates a risk summary,
